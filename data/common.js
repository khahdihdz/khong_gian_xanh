(() => {
  ...
  const CSS = `
    ...
    /* Header cố định bố cục: không wrap, không làm các tab nhảy vị trí. */
    .kgx-header{
      width:100%;box-sizing:border-box;padding:14px 20px;
      border-bottom:1px solid var(--l,#31432c);
      display:flex;align-items:center;justify-content:space-between;
      gap:16px;background:var(--bg,#0c130c);color:var(--t,#f5f8ee);
    ...
    .kgx-nav{position:relative;width:104px;height:40px}
    .kgx-nav-toggle{width:104px;height:40px;border:1px solid var(--l,#31432c);background:var(--p,#182016);border-radius:99px;padding:0 13px;color:var(--t,#f5f8ee);font:inherit;font-size:.8rem;cursor:pointer;white-space:nowrap}
    .kgx-nav-menu{display:none;position:absolute;right:0;top:48px;width:220px;min-width:220px;padding:7px;background:var(--p,#182016);border:1px solid var(--l,#31432c);border-radius:14px;box-shadow:0 14px 35px rgba(0,0,0,.35);z-index:10000}
    .kgx-nav.open .kgx-nav-menu{display:grid;gap:3px}
    .kgx-nav-menu a{display:block;padding:10px 12px;border-radius:9px;color:var(--t,#f5f8ee);text-decoration:none;font-size:.82rem;white-space:nowrap}
    .kgx-nav-menu a:hover,.kgx-nav-menu a:focus{background:var(--p2,#1e2a1a);color:#4ade80;outline:0}.kgx-nav-menu a.active{background:rgba(74,222,128,.14);color:#4ade80}
    ...
    /* Các trang cũ có header/footer riêng: loại bỏ để tránh trùng. */
    body>.bar,body>header.top,body>header.bar,body>.top,body>.heading{display:none !important}
    body>.back-link{display:none !important}

    /* Một số trang cũ dùng body flex để căn giữa; header chung phải nằm toàn chiều ngang. */
    body.kgx-common-layout{display:block !important;padding:0 !important;min-height:100vh}
    body.kgx-common-layout>.kgx-page-content{display:block}

    @media(max-width:760px){
      .kgx-header{padding:12px 14px;display:block}
      .kgx-brand{display:block;width:100%;font-size:1.22rem;margin-bottom:10px}
      .kgx-actions{width:100%;grid-template-columns:minmax(0,1fr) 96px 96px;gap:7px}
      .kgx-pill,.kgx-badge,.kgx-nav,.kgx-nav-toggle{height:38px}
      .kgx-nav{width:96px}.kgx-nav-toggle{width:96px}
      .kgx-nav-menu{position:fixed;right:12px;top:112px;width:min(230px,calc(100vw - 24px));min-width:0}
      .kgx-footer{margin-top:18px;padding:16px 12px}
    }
    @media(max-width:390px){
      .kgx-actions{grid-template-columns:minmax(0,1fr) 88px 88px}
      .kgx-nav,.kgx-nav-toggle{width:88px}
      .kgx-pill,.kgx-badge,.kgx-nav-toggle{font-size:.74rem;padding-left:8px;padding-right:8px}
    }
  `;

  const FALLBACK_HEADER = `
<header class="kgx-header">
  <div class="kgx-brand">🌿 Không Gian Xanh<small>Giám sát môi trường phòng</small></div>
  <div class="kgx-actions">
    <span class="kgx-pill"><span id="dot" class="kgx-dot"></span><span id="conn">Đang kiểm tra MQTT</span></span>
    <span id="clock" class="kgx-pill">--:--:--</span>
    <div class="kgx-nav" id="nav">
      <button class="kgx-nav-toggle" id="navToggle" type="button" aria-expanded="false" aria-controls="navMenu">☰ Menu</button>
      <nav class="kgx-nav-menu" id="navMenu" aria-label="Điều hướng">
        <a href="/">🏠 Tổng quan</a>
        <a href="/mqtt">📡 MQTT</a>
        <a href="/tools">⚙️ Chức năng</a>
        <a href="/wifi">📶 Wi-Fi</a>
        <a href="/update">⬆️ Nâng cấp OTA</a>
      </nav>
    </div>
  </div>
</header>`;
  ...
  function installCss(){
    ...
    const style=document.createElement('style');style.id='kgx-common-style';style.textContent=CSS;document.head.appendChild(style);
  }

  async function loadFragment(path,fallback){
    try{const r=await fetch(path,{cache:'no-store'});if(r.ok){const t=await r.text();if(t.trim())return t;}}catch(e){}
    return fallback;
  }

  function setHeaderState(on,text){
    const dot=document.getElementById('dot'),conn=document.getElementById('conn');
    if(dot)dot.className='kgx-dot'+(on?' on':'');
  ...
  function normalizeRoutes(){
    ...
    const map=esp32
      ?{'/mqtt':'/mqtt_dashboard.html','/tools':'/tools.html','/wifi':'/wifi_config.html','/update':'/update'}
      :{'/mqtt':'/mqtt','/tools':'/tools','/wifi':'/wifi','/update':'/update'};
  ...
  function bindNav(){
    ...
    nav.querySelectorAll('a').forEach(link=>{const href=new URL(link.href,location.href).pathname.replace(/\/$/,'')||'/';if(href===current)link.classList.add('active')});
  }

  function loadMqttLibrary(){
    if(window.mqtt)return Promise.resolve(window.mqtt);
    if(window.__kgxMqttLoader)return window.__kgxMqttLoader;
    window.__kgxMqttLoader=new Promise((resolve,reject)=>{const s=document.createElement('script');s.src='https://unpkg.com/mqtt/dist/mqtt.min.js';s.async=true;s.onload=()=>resolve(window.mqtt);s.onerror=reject;document.head.appendChild(s)});
    return window.__kgxMqttLoader;
  }
  ...
  async function init(){
    installCss();
    document.body.classList.add('kgx-common-layout');

    // Xóa header/footer cũ trước khi chèn bản dùng chung.
    ...
    const header=await loadFragment('/header.html',FALLBACK_HEADER);
    const footer=await loadFragment('/footer.html',FALLBACK_FOOTER);
