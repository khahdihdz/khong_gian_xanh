(() => {
  ...
  function loadMqttLibrary(){
    ...
    window.__kgxMqttLoader=new Promise((resolve,reject)=>{
      const script=document.createElement('script');script.src='https://unpkg.com/mqtt/dist/mqtt.min.js';script.async=true;
      script.onload=()=>resolve(window.mqtt);script.onerror=reject;document.head.appendChild(script);
    });
  ...
  async function connectHeaderMqtt(){
    try{
      ...
      if(!mqtt){setHeaderState(false,'MQTT chưa sẵn sàng');return}
      const port=Number(cfg.port)||8884;
      ...
      client.on('message',(t,p)=>{if(t!==topic)return;try{const d=JSON.parse(p.toString());const ts=Number(d.timestamp);if(Number.isFinite(ts)&&ts>0){const clock=document.getElementById('clock');if(clock)clock.textContent=new Date(ts*1000).toLocaleTimeString('vi-VN')}}catch(e){}});
