...
static bool utf8Next(const char* s, size_t len, size_t& i, uint32_t& cp) {
    ...
    if ((c & 0xE0) == 0xC0 && i < len) {
        cp=((c&0x1F)<<6)|((uint8_t)s[i++]&0x3F); return true;
    }
    if ((c & 0xF0) == 0xE0 && i+1 < len) {
        cp=((c&0x0F)<<12)|(((uint8_t)s[i++]&0x3F)<<6)|((uint8_t)s[i++]&0x3F); return true;
    }
    if ((c & 0xF8) == 0xF0 && i+2 < len) {
        cp=((c&7)<<18)|(((uint8_t)s[i++]&0x3F)<<12)|(((uint8_t)s[i++]&0x3F)<<6)|((uint8_t)s[i++]&0x3F);
        return true;
    }
    cp='?'; return true;
}

// Vietnamese decomposition: base + shape + tone.
// shape: 0 normal, 1 breve (ă), 2 circumflex (â/ê/ô), 3 horn (ơ/ư).
// tone: 0 none, 1 acute, 2 grave, 3 hook, 4 tilde, 5 dot below.
...
static void drawGlyph(int x, int y, char ch, uint8_t shape, uint8_t tone) {
    const uint8_t* g = glyph((uint32_t)ch);
    if (!g) return;
    ...
    // Chữ Đ/đ: thêm gạch ngang đặc trưng.
    if (ch == 'D' || ch == 'd') {
        for (int px=x+1; px<=x+4; ++px) oled.drawPixel(px, y+3, SSD1306_WHITE);
    }

    // Dấu hình dạng: tách rõ ă, â/ê/ô và ơ/ư.
    if (shape == 1) { // ă
        oled.drawPixel(x+1,y-2,SSD1306_WHITE);
        oled.drawPixel(x+2,y-1,SSD1306_WHITE);
        oled.drawPixel(x+3,y-2,SSD1306_WHITE);
    } else if (shape == 2) { // â ê ô
        oled.drawPixel(x+1,y-2,SSD1306_WHITE);
    ...
    // Dấu thanh. Dấu nặng dùng 2x2 pixel để không bị nhìn thành dấu phẩy.
    ...
    if (tone == 1) { // sắc
        ...
        oled.drawPixel(x+1,ty,SSD1306_WHITE);
        oled.drawPixel(x+2,ty+1,SSD1306_WHITE);
        oled.drawPixel(x+3,ty+1,SSD1306_WHITE);
        oled.drawPixel(x+4,ty,SSD1306_WHITE);
    } else if (tone == 5) { // nặng
        oled.drawPixel(x+2,y+7,SSD1306_WHITE);
        oled.drawPixel(x+3,y+7,SSD1306_WHITE);
        oled.drawPixel(x+2,y+8,SSD1306_WHITE);
        oled.drawPixel(x+3,y+8,SSD1306_WHITE);
    }
}
...
static int drawVietnamese(int x, int y, const String& text) {
    ...
    while (i<len) {
        uint32_t cp;
        utf8Next(text.c_str(), len, i, cp);

        char base=0;
        uint8_t shape=0, tone=0;
        if (vnDecompose(cp, base, shape, tone)) cp=(uint32_t)base;

        if (glyph(cp)) drawGlyph(x, y, (char)cp, shape, tone);
        else drawGlyph(x, y, '?', 0, 0);
        x += 6;
    }
    return x;
}
...
void displayUpdate(const SensorData& data,const String& wifiStatus,const String& timeStr) {
    ...
    if (data.sht31Ok) {
        ...
        drawVietnamese(54,27,"Lỗi");
    }

    drawVietnamese(2,37,"AQI:");
    if (data.ens160Ok) drawVietnamese(26,37,data.aqiLabel);
    else drawVietnamese(26,37,"Lỗi");

    oled.setCursor(68,37);
    if (data.ens160Ok) oled.printf("CO2:%d",data.eco2);
    else oled.print("CO2:--");
...
void displayShowError(const String& message) {
    if (!s_oledOk) return;
    oled.clearDisplay();
    drawVietnamese(0,20,"LỖI CẢM BIẾN:");
    drawVietnamese(0,35,message);
    oled.display();
}
