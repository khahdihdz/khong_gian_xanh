...
void sensorRead() {
    if (g_sensorData.sht31Ok) {
        ...
        if (isnan(t) || isnan(h)) {
            Serial.println("[SENSOR] Lỗi đọc SHT31-D! Dữ liệu nhiệt độ/độ ẩm đã được đánh dấu không hợp lệ.");
    ...
    if (g_sensorData.ens160Ok) {
        ...
        if (ens160.checkDataStatus()) {
            ...
            Serial.println("[SENSOR] ENS160 chưa có dữ liệu mới, giữ nguyên mẫu hợp lệ trước đó.");
        }
    } else {
        g_sensorData.aqi = 0;
        g_sensorData.tvoc = 0;
